...
class LibrarySerializerTest(TestCase):
    ...
    def test_valid_data(self):
        serializer = LibrarySerializer(data={'name': 'Test Library', 'city': self.city.name})
    ...
    def test_invalid_city(self):
        serializer = LibrarySerializer(data={'name': 'Test Library', 'city': 'Nonexistent City'})
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors['city'], ['Object with name=Nonexistent City does not exist.'])

    def test_duplicate_name(self):
        Library.objects.create(name='Test Library', city=self.city)
        serializer = LibrarySerializer(data={'name': 'Test Library', 'city': self.city.name})
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors['name'], ['Библиотека with this name already exists.'])
